from internal.transactions.repository.transactions import get_all_transactions, get_transaction, add_transaction, update_transaction, delete_transaction
...
async def get_transactions():
    transactions = await get_all_transactions(session)
    return transactions
...
async def create_transaction(transaction_data:TransactionCreateUpdateModel):
    ...
    transaction = await add_transaction(session, new_transaction)
    return transaction

@router.get("/{transaction_id}")
async def show_transaction(transaction_id):
    category = await get_transaction(session, transaction_id)
    return category
...
async def upd_transaction(transaction_id: str, data: TransactionCreateUpdateModel):
    ...
    await update_transaction(session, transaction_id, data={
...
async def remove_transaction(transaction_id):
    ...
    result = await delete_transaction(session, transaction)
